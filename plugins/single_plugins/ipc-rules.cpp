  ...
  private:
        ...
        [=] (wf::ipc::client_disconnected_signal *ev)
    {
        ...
        if (clients.empty())
        {
            ...
            wf::get_core().disconnect(&on_view_unmapped);
            wf::get_core().disconnect(&on_view_set_output);
            wf::get_core().disconnect(&on_view_geometry_changed);
            wf::get_core().disconnect(&on_view_moved_to_wset);
            wf::get_core().disconnect(&on_kbfocus_changed);
            wf::get_core().disconnect(&on_title_changed);
            wf::get_core().disconnect(&on_app_id_changed);
            wf::get_core().disconnect(&on_plugin_activation_changed);
            wf::get_core().disconnect(&on_output_gain_focus);

            for (auto wo : wf::get_core().output_layout->get_outputs())
            {
                disconnect_output_signals(wo);
            }
        }
    };
    ...
    void connect_output_signals(wf::output_t *output)
    {
    ...
    void disconnect_output_signals(wf::output_t *output)
    {
        output->disconnect(&_tiled);
        output->disconnect(&_minimized);
        output->disconnect(&_fullscreened);
        output->disconnect(&_stickied);
        output->disconnect(&_view_workspace);
        output->disconnect(&on_wset_changed);
        output->disconnect(&on_wset_workspace_changed);
    }
