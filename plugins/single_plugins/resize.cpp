  ...
  public:
    ...
    bool initiate(wayfire_toplevel_view view, uint32_t forced_edges = 0)
    {
        ...
        this->edges = forced_edges ?: calculate_edges(
            wf::from_framebuffer_box(view->get_bounding_box()), get_input_coords());
    ...
    wf::dimensions_t calculate_max_size(wf::dimensions_t min)
    {
        ...
        const auto horizontal_margins = wf::containing_size(wf::dimensionsf_t{
            view->toplevel()->pending().margins.left + view->toplevel()->pending().margins.right, 0.0}).width;
        const auto vertical_margins   = wf::containing_size(wf::dimensionsf_t{
            0.0,
            view->toplevel()->pending().margins.top + view->toplevel()->pending().margins.bottom}).height;
        if (max_size.width > 0)
        {
