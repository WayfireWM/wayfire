...
class wayfire_test_plugin : public wf::per_output_plugin_instance_t
{
  ...
  public:
    ...
    {
        wf::auxilliary_buffer_t render_buffer;
        if (!render_buffer.allocate({buffer_size, buffer_size}, 1.0, {}))
        {
            LOGE("Failed to allocate render buffer!");
            return;
        }

        wf::render_target_t render_target{render_buffer};
        GLuint fb_id = wf::gles::ensure_render_buffer_fb_id(render_target);
