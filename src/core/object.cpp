...
{
    auto it = std::find(callback->connected_to.begin(), callback->connected_to.end(), this);
    if (it != callback->connected_to.end())
    {
        auto last = std::prev(callback->connected_to.end());
        if (it != last)
        {
            std::iter_swap(it, last);
        }

        callback->connected_to.pop_back();
    }
}
