...
TEST_CASE("floating region converts conservatively to framebuffer region")
{
    wf::regionf_t logical{{1.25, 2.5, 3.5, 4.25}};
    auto framebuffer = to_framebuffer_region(logical);
    auto framebuffer_boxes = as_boxes(framebuffer);
    REQUIRE(framebuffer_boxes.size() == 1);
    REQUIRE(framebuffer_boxes[0].x == 1);
    REQUIRE(framebuffer_boxes[0].y == 2);
    REQUIRE(framebuffer_boxes[0].width == 4);
    REQUIRE(framebuffer_boxes[0].height == 5);
}
